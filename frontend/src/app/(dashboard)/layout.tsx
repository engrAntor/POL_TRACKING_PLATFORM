...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    const [bellNotifs, setBellNotifs] = useState<{ id: number; title: string; message: string; created_at: string }[]>([]);
    ...
    useEffect(() => { fetchBellNotifs(); }, [fetchBellNotifs]);
                    ...
                    <div className="flex items-center gap-2 lg:gap-4">
                        ...
                        <div className="relative" ref={notifRef}>
                            ...
                            >
                                ...
                                {bellNotifs.length > 0 && <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-red-500 rounded-full border-2 border-white" />}
                            </button>
                                ...
                                <div className="absolute right-0 top-12 w-[300px] bg-white rounded-2xl shadow-xl border border-gray-100 z-50 overflow-hidden">
                                    ...
                                    <div className="max-h-[280px] overflow-y-auto">
                                        {bellNotifs.length === 0 ? (
                                            ...
                                            const timeAgo = mins < 1 ? 'Just now' : mins < 60 ? `${mins} min ago` : mins < 1440 ? `${Math.floor(mins / 60)} hr ago` : `${Math.floor(mins / 1440)}d ago`;
                                            return (
                                    ...
                                    <button
                                        ...
                                        className="w-full px-4 py-2.5 text-center text-sm font-medium hover:bg-gray-50 transition-colors"
                                        style={{ color: '#0E3B1F' }}
                                    >
                                        See All
                                    </button>
                        ...
                        <div className="relative" ref={profileRef}>
                            ...
                            {showProfile && (
                                <div className="absolute right-0 top-14 w-[220px] bg-white rounded-2xl shadow-xl border border-gray-100 z-50 overflow-hidden">
                                    ...
                                    {/* Change Password row */}
                                    <button
                                        onClick={() => { setShowProfile(false); setShowPassword(true); setOldPw(''); setNewPw(''); setConfirmPw(''); setPwMsg(''); }}
                                        className="flex items-center justify-between w-full px-5 py-3 text-sm text-gray-700 hover:bg-gray-50 transition-colors"
                                    >
